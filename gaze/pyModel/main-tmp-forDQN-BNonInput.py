...
if GAME_NAME == 'seaquest':
    ...
    MODEL_DIR = "Image_past4/Seaquest_deconv"
    ...
    MODEL_DIR = "Image_past4/Mspacman_deconv"
    ...
    MODEL_DIR = "Image_past4/Centipede_deconv"
    ...
    MODEL_DIR = "Image_past4/Freeway_deconv"
    ...
    MODEL_DIR = "Image_past4/Venture_deconv"    
    ...
    MODEL_DIR = "Image_past4/Riverraid_deconv"
    ...
    MODEL_DIR = "Image_past4/Enduro_deconv"
    ...
    MODEL_DIR = "Image_past4/Breakout_deconv" 

MODEL_DIR = "DQN_" + MODEL_DIR # hacky (temporary) way

LABELS_FILE_TRAIN = BASE_FILE_NAME + '-train_first_100.txt' 
LABELS_FILE_VAL =  BASE_FILE_NAME + '-train_first_100.txt' 
...
SHAPE = (84,84,4) # height * width * channel This cannot read from file and needs to be provided here


if not predict_mode: # if train
    import input_utils as IU, misc_utils as MU
    expr = MU.BMU.ExprCreaterAndResumer(MODEL_DIR, postfix='pKf_dp%.2f_BNonInput' % (dropout))
    expr.redirect_output_to_logfile_if_not_on("eldar-11")
else:
    assert False, "Todo: change code here"
...
output = L.Activation(MU.my_softmax,name="prob")(x)
xBN = L.Multiply()([inputs,output])
xBNbefore = L.Concatenate(name='xBNbefore')([inputs,xBN])
xBNafter = L.BatchNormalization(center=False, scale=False,epsilon=0.000001, name='xBNafter')(xBNbefore)

model=Model(inputs=inputs, outputs=[output, xBNbefore, xBNafter])

opt=K.optimizers.Adadelta(lr=1.0, rho=0.95, epsilon=1e-08, decay=0.0)
#opt=K.optimizers.Adam(lr=lr, beta_1=0.9, beta_2=0.999, epsilon=1e-08) 

model.compile(loss={"prob":MU.my_kld, "xBNbefore":None, 'xBNafter':None}, optimizer=opt, metrics=[MU.NSS])

d=IU.DatasetDQN_withGHmap(LABELS_FILE_TRAIN, LABELS_FILE_VAL, GAZE_POS_ASC_FILE)
