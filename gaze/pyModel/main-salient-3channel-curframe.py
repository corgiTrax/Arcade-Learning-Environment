...
if GAME_NAME == 'seaquest':
    ...
    MODEL_DIR = "3Channel/Seaquest_3channel"
    ...
    MODEL_DIR = "3Channel/Mspacman_3channel"
    ...
    MODEL_DIR = "3Channel/Centipede_3channel"
    ...
    MODEL_DIR = "3Channel/Freeway_3channel"
    ...
    MODEL_DIR = "3Channel/Venture_3channel"    
    ...
    MODEL_DIR = "3Channel/Riverraid_3channel"
    ...
    MODEL_DIR = "3Channel/Enduro_3channel"
    ...
    MODEL_DIR = "3Channel/Breakout_3channel" 

LABELS_FILE_TRAIN = BASE_FILE_NAME + '-train.txt' 
LABELS_FILE_VAL =  BASE_FILE_NAME + '-val.txt' 
GAZE_POS_ASC_FILE = BASE_FILE_NAME + '.asc'
PREDICT_FILE_VAL = BASE_FILE_NAME.split('/')[-1] + '-3channel_curframe'
BATCH_SIZE = 50
num_epoch = 70
...
if not predict_mode: # if train
    ...
    expr = MU.BMU.ExprCreaterAndResumer(MODEL_DIR, postfix='current_dp%.1f' % (dropout))
...
if resume_model:
    ...
    # Channel 3: bottum up saliency
    z_inputs=L.Input(shape=SHAPE)
    z=z_inputs # inputs is used by the line "Model(inputs, ... )" below
    
    conv31=L.Conv2D(32, (8,8), strides=4, padding='valid')
    z = conv31(z)
    z=L.Activation('relu')(z)
    z=L.BatchNormalization()(z)
    z=L.Dropout(dropout)(z)
    
    conv32=L.Conv2D(64, (4,4), strides=2, padding='valid')
    z = conv32(z)
    z=L.Activation('relu')(z)
    z=L.BatchNormalization()(z)
    z=L.Dropout(dropout)(z)
    
    conv33=L.Conv2D(64, (3,3), strides=1, padding='valid')
    z = conv33(z)
    z=L.Activation('relu')(z)
    z=L.BatchNormalization()(z)
    z=L.Dropout(dropout)(z)
    
    deconv31 = L.Conv2DTranspose(64, (3,3), strides=1, padding='valid')
    z = deconv31(z)
    z=L.Activation('relu')(z)
    z=L.BatchNormalization()(z)
    z=L.Dropout(dropout)(z)

    deconv32 = L.Conv2DTranspose(32, (4,4), strides=2, padding='valid')
    z = deconv32(z)
    z=L.Activation('relu')(z)
    z=L.BatchNormalization()(z)
    z=L.Dropout(dropout)(z)         

    deconv33 = L.Conv2DTranspose(1, (8,8), strides=4, padding='valid')
    z = deconv33(z)
    print deconv33.output_shape
    z = L.Activation('relu')(z)
    z_output = L.BatchNormalization()(z)


    # Merge outputs from 3 channels
    outputs = L.Average()([x_output, y_output, z_output])
...
opf=IU.Dataset_OpticalFlow(LABELS_FILE_TRAIN, LABELS_FILE_VAL, SHAPE)
bu=IU.Dataset_BottomUp(LABELS_FILE_TRAIN, LABELS_FILE_VAL, SHAPE)

if not predict_mode: # if train
    expr.dump_src_code_and_model_def(sys.argv[0], model)

    model.fit([d.train_imgs, opf.train_flow, bu.train_bottom], d.train_GHmap, BATCH_SIZE, epochs=num_epoch,
        validation_data=([d.val_imgs, opf.val_flow, bu.val_bottom], d.val_GHmap, d.val_weight),
    ...
    score = model.evaluate([d.val_imgs,opf.val_flow,bu.val_bottom], d.val_GHmap, BATCH_SIZE, 0, sample_weight = d.val_weight)
    ...
    #train_score = model.evaluate([d.train_imgs,opf.train_flow,bu.train_bottom], d.train_GHmap, BATCH_SIZE, 0, d.train_weight)
    val_score = model.evaluate([d.val_imgs,opf.val_flow,bu.val_bottom],d.val_GHmap,BATCH_SIZE,0,d.val_weight)
    ...
    val_pred = model.predict([d.val_imgs,opf.val_flow,bu.val_bottom], BATCH_SIZE)
    print "Predicted."
