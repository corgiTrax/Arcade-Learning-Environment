...
from plot import save_heatmap_png_files
from input_utils import frameid_from_filename, read_gaze_data_asc_file, read_result_data, read_heatmap
RESIZE_SHAPE = (84,84)
...
if __name__ == "__main__":
    #if len(sys.argv) < 4:
    #    print "Usage: %s saved_frames_png_tar asc_file result_file_txt result_heatmap_file_npz" % (sys.argv[0])
    #    sys.exit(0)

    # the two parameters to take
    DATASET_NAME = '36_RZ_4882422_May-15-16-08-32'
    # DATASET_NAME = '37_RZ_4883794_May-15-16-37-01'
    RESULT_FILE_NAME = 'cat{36_RZ}tr_{37_RZ}val-train-result'
    # RESULT_FILE_NAME = 'cat{36_RZ}tr_{37_RZ}val-val-result'


    heatmap_path = '../../dataset_gaze/' + RESULT_FILE_NAME + '.npz'
    save_path = '../../saliency/' + DATASET_NAME
    save_heatmap_png_files(heatmap_path, save_path)

    tar = tarfile.open('../../dataset_gaze/' + DATASET_NAME + '.tar.bz2', 'r')
    asc_path = '../../dataset_gaze/' + DATASET_NAME + '.asc'
    ...
    while ds.cur_frame_id < ds.total_frame:
        ...
        try:
            heatmap = pygame.image.load("/scratch/cluster/zharucs/saliency/" + png_files[ds.cur_frame_id])
        ...
        heatmap_array = pygame.surfarray.pixels3d(heatmap)

        overlay_img = overlay(sarray, heatmap_array, 0.6)
        overlay_img_surface = pygame.surfarray.make_surface(overlay_img)
        # overlay_img_surface = pygame.transform.scale(overlay_img_surface, (w,h))
        
        screen.blit(overlay_img_surface, (0,0))
