...
def old_ver_create_for_only_one_trial():
    ...
    frameid2pos, frameid2action = read_gaze_data_asc_file(asc_file)
    ...
    for png in png_files:
        ...
        if fid in frameid2action and frameid2action[fid] != None:
            if fid in frameid2pos and frameid2pos[fid]:
                l = len(frameid2pos[fid])
                xy_str.append('%s %d %f %f' % (png, frameid2action[fid], frameid2pos[fid][l-1][0], frameid2pos[fid][l-1][1]))
            else:
