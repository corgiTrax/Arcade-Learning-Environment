...
if resume_model:
    ...
    conv1=L.Conv2D(32, (8,8), strides=1, padding='same')
    x = conv1(x)
    print conv1.output_shape
    x=L.Activation('relu')(x)
    x=L.BatchNormalization()(x)   
    x=L.MaxPooling2D(pool_size=(3, 3),strides=2,padding='same')(x)
    x=L.Dropout(dropout)(x)

    conv2=L.Conv2D(64, (4,4), strides=1, padding='same')
    x = conv2(x)
    ...
    conv3=L.Conv2D(128, (3,3), strides=1, padding='same')
    x = conv3(x)
    ...
    conv4=L.Conv2D(128, (5,5), strides=1, padding='same')
    x = conv4(x)
    ...
    deconv4 = L.Conv2DTranspose(1, (4,4), strides=2, padding='same')
    x = deconv4(x)
