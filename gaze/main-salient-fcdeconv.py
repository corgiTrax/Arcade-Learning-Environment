...
if resume_model:
    ...
    x = L.Dropout(dropout)(x)
    x = L.Dense(3136, activation = "relu")(x)
    x = L.Dropout(dropout)(x)
    x = L.Reshape((7, 7, 64))(x)

    deconv1 = L.Conv2DTranspose(64, (3,3), strides=1, padding='valid', activation='relu')
    x = deconv1(x)
    print deconv1.output_shape
    x=L.Dropout(dropout)(x)

    deconv2 = L.Conv2DTranspose(64, (4,4), strides=2, padding='valid', activation='relu')
    x = deconv2(x)
