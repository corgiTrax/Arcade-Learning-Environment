...
dataset_regex = re.compile('([^ ]+) <-- sys.argv')
acc_regex = re.compile('^(\d+)_(.*?)_?(dr0\.\d+)_?([^ ]+)? (0\.\d+)')
filter_regex = re.compile(sys.argv[1] if len(sys.argv) == 2 else '.*')

# Collect data from stdin and build the data structure 
# Read code below to know the structrue of this data structure
...
for line in sys.stdin:
    ...
    if datasetname_match:
        datasetname=datasetname_match.group(1)
    if acc_match:
        ...
        dataset[datasetname][modelname][dropout].append(float(acc))
